...
void dma1_stream6_init(uint32_t src,uint32_t dst,uint32_t len){
	//enable the clock access to DMA.
	RCC->AHB1ENR |= DMA1EN;
	// Disable DMA1 stream6.
	DMA1->S6CR &=~(1<<DMA_S_EN);
	//Clear all interrupt flags of stream6.
	while(DMA1->S6CR & DMA_S_EN);
	DMA1->HIFCR|=(1U<<16);
	DMA1->HIFCR|=(1U<<18);
	DMA1->HIFCR|=(1U<<19);
	DMA1->HIFCR|=(1U<<20);
	...
	DMA1->S6PAR=dst;
	//set the source buffer to which the data has to be stored.
	DMA1->S6M0AR=src;
	// set length
	DMA1->S6NDTR=len;
	// select stream6 CH4
	DMA1->S6CR|=(1<<27);//selecting the channel.
	...
	//DMA interrupt enable in N VIC.
	GPIO_IRQInterruptConfig(DMA1_Stream6_IRQn,ENABLE);
}
